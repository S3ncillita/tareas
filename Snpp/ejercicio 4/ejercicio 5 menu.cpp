/*Ejemplo 5, funciones en las que usamos menú */ 
#include <stdio.h> 
#include <conio.h> 
#include <math.h> 
#include <stdlib.h> 
...
int suma(int n); /*Declaro todas las funciones antes de main y deben llevar (;)*/ 
/*FUNCION MENU: MUESTRA UN MENU EN PANTALLA*/ 
char menu() 
{ 
printf("\n\n\n\t\tBIENVENIDO/A\n\n"); printf("\n\n\t\tMENU DE OPCIONES\n\n"); 
printf("\n********************************************************\n"); printf("\n\t1-RAIZ CUADRADA DE DOS NUMEROS ENTEROS"); printf("\n\t2-VALOR ABSOLUTO DE DOS NUMEROS ENTEROS"); 
printf("\n\t3-SUMA DE LOS PRIMEROS NUMEROS ENTEROS"); printf("\n\n\t4-SALIR"); printf("\n\n\tESCOJA UNA OPCION"); 
printf("\n********************************************************\n"); 
}
...
do 
{ /*segundo ciclo do while para el switch*/ system("CLS()"); /*limpia pantalla*/
 menu(); 
...
printf("\n\t\tRAIZ CUADRADA DE DOS NUMEROS ENTEROS\n\n"); printf("\nINGRESE EL VALOR DE A, POR FAVOR: "); scanf("%f", &a); 
printf("\nAHORA EL VALOR DE B: "); 
scanf("%f", &b); 
...
printf("\nENTRE EL VALOR DE N: " ); 
scanf("%d",&n); 
printf("\nLA SUMA DE LOS %d PRIMEROS NUMEROS ENTEROS ES: %d", n, suma(n)); 
MENSAJE; 
break; 
case '4':system("CLS"); /*limpia pantalla*/ 
default:printf("\n\n\n\n\tERROR<<<<<DEBES ELEGIR UNA OPCION VALIDA"); break; 
} 
} while(c!='4'); /*fin del segundo ciclo do while para el switch*/ printf("\n\nQUIERES SALIR DEL PROGRAMA?"); 
SI; 
NO; 
...
double y; x=a+b; 
y=sqrt(x); /*raiz cudrada pertenece a la libreria math*/ return (y); /*funcion que devuelve un valor*/ 
} 
double valor_absoluto(float a, float b) 
{ 
float x; /*declaracion de variables locales*/ double y; /*es un tipo de dato que maneja valores + y ‐ */ x=a-b; 
y=fabs(x); /*valor absoluto pertenece a la libreria math*/ return (y); /*funcion que devuelve un valor*/ 
} 
int suma(int n) 
{ 
int i,sum=0; /*declaracion de variables locales*/ for(i=1; i<=n; i++) sum+=i; 
